...
def create_custom_model(model_type, num_classes, dropout_rate):
...
def train_custom_model(
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_acc = evaluate_custom_model(model, train_loader, criterion)
        train_losses.append(train_loss)
        train_accs.append(train_acc)

        # Validation phase
        val_loss, val_acc = evaluate_custom_model(model, val_loader, criterion)
        ...
        if early_stopping(val_losses, patience=2):
...
def evaluate_custom_model(model, data_loader, criterion):
    model.eval()
    running_loss = 0.0
    correct = 0
    total = 0
...
def predict_and_evaluate(model, test_loader, model_type, class_names=None):
    if model_type == "pretrained":
        # For HuggingFace models, we use their prediction mechanism
        trainer = model
        ...
        # For custom models, we use our own prediction mechanism
        model.eval()
        all_preds = []
        all_labels = []
    ...
    fig = plot_confusion_matrix(labels, preds, class_names=class_names)
    cm_path = f"./results/confusion_matrix_{model_type}.png"
...
# ------------------- Optuna Objective -------------------
def objective(trial):
    # First decide whether to use custom CNN or pretrained model
    model_family = trial.suggest_categorical("model_family", ["custom", "pretrained"])
    ...
    tb_logdir = f"runs/trial_{trial_num}"
    writer = SummaryWriter(tb_logdir)
    
    # Different setup based on the model family
    if model_family == "custom":
        # Custom CNN specific parameters
        ...
        # Data transformation for custom CNNs
        ...
        train_dataset = CustomImageDataset(
            IMAGE_DIR, f"{DATASET_DIR}/train_labels.csv", transform
        )
        val_dataset = CustomImageDataset(
            IMAGE_DIR, f"{DATASET_DIR}/val_labels.csv", val_transform
        )
        ...
        # Create model
        model = create_custom_model(model_type, NUM_CLASSES, dropout_rate)
        logger.info(
        ...
        with mlflow.start_run():
            ...
            val_accuracy = train_custom_model(
                model,
                train_loader,
                val_loader,
                optimizer,
            ...
            metrics = predict_and_evaluate(model, val_loader, "custom", class_names=None)
            mlflow.log_metrics({
            ...
            # Save model
            model_path = f"./models/model_trial_{trial_num}.pth"
        ...
        try:
            ...
            train_dataset = HuggingFaceImageDataset(
                IMAGE_DIR, f"{DATASET_DIR}/train_labels.csv", processor
            )
            val_dataset = HuggingFaceImageDataset(
                IMAGE_DIR, f"{DATASET_DIR}/val_labels.csv", processor
            )
            
            # Set up training arguments
            training_args = TrainingArguments(
                output_dir=f"./results/trial_{trial_num}",
                save_strategy="no",
                learning_rate=learning_rate,
            ...
            trainer = Trainer(
                ...
                callbacks=[LiveMetricsLogger(writer, trial_num)]
            )
            ...
            with mlflow.start_run():
                ...
                logger.info(f"Training pretrained model: {model_name}")
                trainer.train()
                metrics = trainer.evaluate()
                
                mlflow.log_metrics({
                    "val_accuracy": metrics["eval_accuracy"],
                    "val_f1_score": metrics["eval_f1"]
                })
                
                # Get predictions and create confusion matrix
                eval_metrics = predict_and_evaluate(trainer, val_dataset, "pretrained", class_names=None)
                mlflow.log_artifact(eval_metrics["confusion_matrix_path"])
                
                val_accuracy = metrics["eval_accuracy"]
        
        except Exception as e:
            logger.error(f"Error training pretrained model {model_name}: {e}")
            # Return a very low value to indicate failure
            val_accuracy = 0.0
    
    writer.close()
    logger.info(f"Finished trial {trial_num} with accuracy: {val_accuracy:.4f}")
    return val_accuracy

# ------------------- Main -------------------
if __name__ == "__main__":
    # Configuration
    NUM_CLASSES = 120  # Update based on your dataset
    IMAGE_DIR = "data_2/Images"
    DATASET_DIR = "dataset"
    
    # Setup MLflow
    mlflow.set_tracking_uri("file:./mlruns")
    mlflow.set_experiment("combined_nas_image_classification")
    
    # Create dataset CSVs
    logger.info("Creating CSVs from image folder...")
    create_label_csv(IMAGE_DIR, DATASET_DIR)
